"""Synthetic-only tests of the assembled Session 14R7 study path."""
...
spec=importlib.util.spec_from_file_location('r7',ROOT/'scripts/session_14r7_occlusion_study.py')
runner=importlib.util.module_from_spec(spec);spec.loader.exec_module(runner)


class Session14R7StudyTests(unittest.TestCase):
    ...
    def test_inherited_pair_aggregation_blocks_access(self):
        result=runner.summary_contract_check()
        self.assertFalse(result['passed'])
        self.assertEqual(result['observed']['mean'],result['expected']['mean'])
        self.assertEqual((result['observed']['q50'],result['expected']['q50']),(1.,.5))
        with self.assertRaisesRegex(Exception,'pair_summary_aggregation_contract'):
    ...
    def test_no_prohibited_routes_and_path_escape(self):
        source=(ROOT/'scripts/session_14r7_occlusion_study.py').read_text()
