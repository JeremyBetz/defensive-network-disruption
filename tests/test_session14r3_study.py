...
spec=importlib.util.spec_from_file_location('r3',ROOT/'scripts/session_14r3_occlusion_study.py')
runner=importlib.util.module_from_spec(spec);spec.loader.exec_module(runner)

class Session14R3Tests(unittest.TestCase):
...
class R3GovernanceTests(unittest.TestCase):
    ...
    def test_synthetic_summary_end_to_end(self):
        ...
        with tempfile.TemporaryDirectory() as tmp,patch.object(runner,'ROOT',Path(tmp).resolve()),patch.object(runner,'evaluate_edge',side_effect=fake):
    ...
    def test_empirical_helpers_require_single_launch(self):
        with patch.dict(runner.ACCESS,authorized=False):
            with self.assertRaises(PermissionError):runner.prepare()
            with self.assertRaises(PermissionError):runner.analyze()

    def test_startup_import_failure_closes_without_runtime(self):
        with tempfile.TemporaryDirectory() as tmp,patch.object(runner,'ROOT',Path(tmp).resolve()):
            for path in (*runner.CODE,runner.PROTOCOL):
                dest=runner.ROOT/path;dest.parent.mkdir(parents=True,exist_ok=True);dest.write_text('synthetic')
            runner.startup_close(ImportError('synthetic unavailable runtime'))
            value=runner.load(runner.OUT/'manifest.json')
            self.assertEqual(value['status'],'blocked')
            self.assertEqual(value['environment'],{'unavailable':'runtime_import_failure'})
            self.assertFalse(runner.load(runner.OUT/'qc.json')['empirical_access'])
            self.assertIn('candidate_summary.json',value['unavailable'])

    def test_fixed_cli_has_only_one_access_route(self):
        import inspect
        source=inspect.getsource(runner.main)
        self.assertNotIn("'prepare':",source);self.assertNotIn("'analyze':",source)
        self.assertIn("'run':run",source)

    def test_partially_collected_outputs_rejected(self):
        c=runner.Collect();c.add('match_summary.csv',runner.ALIASES[0],0,'field',[1.])
        with tempfile.TemporaryDirectory() as tmp,patch.object(runner,'ROOT',Path(tmp).resolve()):
            runner.write_csv('match_summary.csv',c.tables()['match_summary.csv'])
            with self.assertRaises(pv.GateFailure):runner.validate_payloads(['match_summary.csv'],'blocked')

    def test_run_startup_failure_closes_before_population(self):
        with tempfile.TemporaryDirectory() as tmp,patch.object(runner,'ROOT',Path(tmp).resolve()),patch.object(runner,'preflight'),patch.object(runner,'observe',side_effect=RuntimeError('synthetic startup')),patch.object(runner,'digest',return_value='hash'),patch.object(runner,'code_hashes',return_value={}),patch.object(runner,'environment',return_value={}),patch.object(runner,'ledger'),patch.object(runner,'prepare') as prepare:
            with self.assertRaises(RuntimeError):runner.run()
            prepare.assert_not_called()
            self.assertEqual(runner.load(runner.OUT/'manifest.json')['status'],'blocked')
            self.assertFalse(runner.load(runner.OUT/'qc.json')['empirical_access'])
