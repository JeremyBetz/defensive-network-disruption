#!/usr/bin/env python3
"""One-shot Session 14R9E label-free empirical execution."""
...
from defensive_network_disruption.geometry.r9e_representation import evaluate_edge
...
from defensive_network_disruption.validation.r9e_publication import derive_progress, validate_public


START = "fc7a4da62a64e597acbeccd790f825b6fd35e26c"
...
PROTOCOL = Path("docs/protocols/phase_14r9e_empirical_execution.md")
OUT = Path(os.environ.get("SESSION14R9E_OUTPUT", "outputs/continuous_occlusion_empirical_retry_r9e"))
LOCAL = OUT / "local"
R9 = Path("outputs/continuous_occlusion_empirical_retry_r9")
R9A = Path("outputs/continuous_occlusion_empirical_retry_r9a")
TEST_COMMAND = ("-m", "unittest", "discover", "-s", "tests", "-p", "test_session14r9e.py")
INHERITED_PUBLIC = (
...
def preflight() -> dict:
    ...
    for marker in (LOCAL / "empirical_attempt.marker", LOCAL / "preparation.marker", LOCAL / "execution.marker"):
    ...
    return {"schema_version": 1, "status": "ready", "start": START,
            ...
            "environment": environment(), "inherited": authority}
...
def verify_ci() -> dict:
    ...
    raw = subprocess.check_output(("gh", "run", "list", "--commit", head, "--workflow", "ci.yml",
                                   "--json", "databaseId,headSha,status,conclusion", "--limit", "10"), cwd=ROOT)
    runs = json.loads(raw)
    good = [item for item in runs if item["headSha"] == head and item["status"] == "completed" and item["conclusion"] == "success"]
    if not good:
        raise RuntimeError("preaccess_ci_unavailable")
    ...
    if not all(jobs.get(name) == "success" for name in ("test (3.11)", "test (3.13)", "distribution")):
        raise RuntimeError("preaccess_ci_jobs")
    return {"head": head, "run": good[0]["databaseId"], "jobs": jobs}
...
def _private_package(authority, status, stage, exception, staging=None) -> Path:
    ...
    if staging is not None:
        for name in (*CSV_FILES, "candidate_summary.json"):
            atomic_copy(staging / name, package / name)
    acceptance = numerical_publication.package(
        numerical_publication.authority(PROGRESS.journal.path, PROGRESS.journal.previous),
        accepted=status == "success", checks={"complete_processing": status == "success"})
    for name in ("qc", "manifest", "evidence"):
        durable_write(safe(package / "numerical_publication" / (name + ".json")), acceptance)
    durable_write(safe(package / "progress_authority.json"), authority)
    base = {"schema_version": 1, "status": status, "progress_authority": authority}
...
def publish(authority, status, stage, exception, staging=None) -> dict:
    package = _private_package(authority, status, stage, exception, staging)
    if staging is not None:
        for name in (*CSV_FILES, "candidate_summary.json"):
            atomic_copy(staging / name, OUT / name)
    ...
    durable_write(safe(OUT / "qc.json"), {"schema_version": 1, "status": status,
        "progress_authority": authority, "stage": stage, "exception": exception,
        "scientific_comparisons_available": status == "success"})
    available = [name for name in FINAL_FILES if safe(OUT / name).exists()]
    durable_write(safe(OUT / "manifest.json"), {"schema_version": 1, "status": status,
        "progress_authority": authority, "start": START, "protocol": sha(PROTOCOL),
    ...
    return validate_public(safe(OUT), private_dir=safe(package), journal=PROGRESS.journal.path,
                           expected_head=PROGRESS.journal.previous, mode=mode)


def close_failure(stage: str, error: BaseException) -> None:
    if safe(OUT / "manifest.json").exists():
        return
    trace = "".join(traceback.format_exception(error))
    if not safe(LOCAL / "original_failure.json").exists():
        durable_write(safe(LOCAL / "original_failure.json"), {"schema_version": 1,
    ...
    if PROGRESS is None:
        durable_write(safe(LOCAL / "emergency_failure.json"), {"schema_version": 1,
            "status": "failure", "stage": stage, "exception": type(error).__name__,
            "traceback": trace, "progress_authority": None})
        return
    try:
        PROGRESS.failure(stage, error)
        authority = derive_progress(PROGRESS.journal.path,
            expected_head=PROGRESS.journal.previous, mode="empirical_failure")
        publish(authority, "failure", stage, type(error).__name__)
    except BaseException as publication_error:
        if not safe(LOCAL / "emergency_failure.json").exists():
            durable_write(safe(LOCAL / "emergency_failure.json"), {"schema_version": 1,
                "status": "failure", "stage": stage, "exception": type(error).__name__,
...
def run() -> None:
    ...
    try:
        ...
        def verify(_context):
            ...
            ci = verify_ci(); durable_write(safe(LOCAL / "preaccess_authority.json"), {**bindings, "ci": ci})
            return True
        ...
        def access(_context):
            ...
            stage = "publication"
            authority = derive_progress(PROGRESS.journal.path,
                expected_head=PROGRESS.journal.previous, mode="empirical_success")
            receipt = publish(authority, "success", "closed", None, staging)
        ...
        print("R9E empirical execution closed and publication validated")
    except BaseException as error:
        close_failure(stage, error)
...
def publication_check() -> dict:
    ...
    return validate_public(safe(OUT), private_dir=safe(package), journal=safe(LOCAL / "journal.jsonl"),
        expected_head=manifest["progress_authority"]["journal_sha256"], mode=mode)
