#!/usr/bin/env python3
"""Governed Session 14R8, explicit label-free geometry and immutable stages."""
...
START='e47ab1524206a5d42c3c6106b1955913b4435120'
...
OUT=Path('outputs/continuous_occlusion_retry_14r8'); LOCAL=OUT/'local'; AUTH=OUT/'authority'
PROTOCOL=Path('docs/protocols/phase_14r8_continuous_occlusion_empirical_retry.md')
CODE=('scripts/session_14r8_occlusion_study.py',
      'src/defensive_network_disruption/validation/r8_aggregation.py',
      'src/defensive_network_disruption/validation/r7_execution.py',
      'tests/test_session14r8_enforcement.py','tests/test_session14r8_study.py',
...
FINAL_FILES=('candidate_summary.json',*CSV_FILES,'synthetic_stress_summary.json','lifecycle_summary.json','exposure_summary.json','pair_aggregation_summary.json','qc.json','synthetic_field_comparison.svg')
...
TEST_COMMAND=('-m','unittest','discover','-s','tests','-p','test_session14r8_enforcement.py')
PROGRESS=None
...
def render_synthetic(approve=False):
    ...
    try:
        ...
        def render():
            with matplotlib.rc_context({'svg.hashsalt':'phase14r8-fixed','font.family':'DejaVu Sans','font.size':11}):
                ...
                buf=io.StringIO();fig.savefig(buf,format='svg',metadata={'Date':None,'Creator':'Session 14R8 synthetic illustration'});plt.close(fig)
...
def summarize_state(row,state_index,collector,across):
    ...
    add('receiver_segment_divergence.csv','raw_order_disagreement',[float((i,j) in discord) for i,j in validpairs],unit='state_pair_proportion')
    fields={};qc=[]
    for candidate in CANDIDATES:
        ...
        for summary in ('receiver','segment'):
            ...
            for combination,vals in (('union',uv),('maximum',mv)):
                ...
                for metric,vs in follows(vals,pairs).items():add('receiver_segment_divergence.csv',metric,vs,unit='state_pair_proportion',**kw)
    ...
    for candidate in CANDIDATES:
        for combination in ('union','maximum'):
            ...
            for metric,vs in order_categories(a,b).items():add('receiver_segment_divergence.csv',metric,vs,unit='state_pair_proportion',**kw)
...
def preflight():
    ...
    print('R8 protocol and historical authority verified; no development input opened')
...
def verify_production():
    ...
    try:
        ...
        print('108/366/399 assembled R8 acceptance passed')
...
def publication_check():
    ...
    if m['status']=='success':
        pair=load(OUT/'pair_aggregation_summary.json')
        pv.require(pair['status']=='state_first_pair_aggregation_applied','pair_summary_status')
    print('R8 persisted publication validated: '+m['status'])
