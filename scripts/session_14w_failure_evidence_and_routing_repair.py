#!/usr/bin/env python3
"""Session 14w bounded micro-interval verifier and single-edge regression."""
...
START = "348ccdea71df1fc06df04440ef457f2a87daf849"
TAG = "f00690c05d8c1c6db308a65bd311c43f9a8ef2fa"
PROTOCOL = Path("docs/protocols/phase_14w_failure_evidence_and_routing_repair.md")
OUT = Path("outputs/continuous_occlusion_routing_repair")
LOCAL = OUT / "local"
...
CODE = (
    Path("scripts/session_14w_failure_evidence_and_routing_repair.py"),
    Path("src/defensive_network_disruption/geometry/micro_interval_verifier.py"),
    Path("tests/test_session14w_failure_evidence.py"),
)
PUBLIC = (
    "contract.json", "synthetic_routing_oracles.csv", "failure_publication_oracles.json",
    "access_accounting_oracles.csv", "synthetic_reference_regression.csv",
    "failing_edge_routing.json", "failing_edge_regression.json", "qc.json",
)
REFERENCE_COLUMNS = (
...
def preservation() -> None:
    ...
    allowed = {str(PROTOCOL), *map(str, CODE),
               "docs/session_14w_micro_interval_verifier.md", "docs/research_log.md"}
...
def preflight() -> None:
    ...
    print("Session 14w preflight passed; no numerical execution or authorized edge access")
...
def edge_regression():
    ...
    threshold = repaired.GLOBAL_RESIDUAL_BUDGET / len(widths)
    eligible = [width for width in widths if width <= threshold]
    routing = {
        "schema_version": 1, "status": "success", "structural_piece_count": len(widths),
        "positive_width_piece_count": len(widths), "eligibility_threshold": threshold,
        "bounded_residual_widths": eligible, "bounded_residual_piece_count": len(eligible),
        "quadrature_piece_count": len(widths)-len(eligible),
        "total_residual_lower": 0.0, "total_residual_upper": math.fsum(eligible),
        "deterministic_routing": eligible == [width for width in widths if width <= threshold],
    }
    atomic_json("failing_edge_routing.json", routing)
    atomic_json("routing_detail.json", {"pieces":[{"ordinal": i,"left":a,"right":b,
        "width":b-a,"threshold":threshold,"eligible":b-a<=threshold,
        "routing":"bounded_residual" if b-a<=threshold else "adaptive_quadrature"}
        for i,(a,b) in enumerate(zip(partitions[:-1],partitions[1:],strict=True))]}, private=True)
    historical.require(routing["total_residual_upper"] <= repaired.GLOBAL_RESIDUAL_BUDGET and
                       routing["deterministic_routing"], "routing_rule_failed")
    ...
    historical.require(interval.structural_piece_count == 12 and
                       interval.bounded_piece_count == len(eligible) and
                       interval.quadrature_piece_count == 12-len(eligible), "routing_record_mismatch")
    ...
    return {
        ...
        "known_micro_width": micro_width, "eligibility_threshold": threshold,
        "bounded_residual_widths": eligible, "bounded_piece_count": interval.bounded_piece_count,
...
def audit() -> None:
    ...
    try:
        ...
        write_csv("synthetic_routing_oracles.csv", ORACLE_COLUMNS, oracle_rows)
        write_json("failure_publication_oracles.json", {"schema_version":1,"status":"success",
            "before_routing":True,"after_routing":True,"quadrature":True,"comparison":True,"closure":True})
        write_csv("access_accounting_oracles.csv", ("fixture","states","edges","blocked"), [
            {"fixture":"before_access","states":0,"edges":0,"blocked":False},
            {"fixture":"state_only","states":1,"edges":0,"blocked":False},
            {"fixture":"edge_failure","states":1,"edges":1,"blocked":False},
            {"fixture":"success","states":1,"edges":1,"blocked":False},
            {"fixture":"unauthorized_second","states":1,"edges":1,"blocked":True}])
        ...
        print("Session 14w audit closed: A, readiness 1")
...
def publication_check() -> None:
    ...
    print(f"Session 14w publication checks passed: {manifest['status']}")
