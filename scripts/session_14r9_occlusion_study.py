#!/usr/bin/env python3
"""Governed Session 14R9, explicit label-free geometry and immutable stages."""
...
START='d7a66699f133c36a6148303d3f420336473e58ce'
...
OUT=Path(os.environ.get('SESSION14R9_OUTPUT','outputs/continuous_occlusion_empirical_retry_r9')); LOCAL=OUT/'local'; AUTH=OUT/'authority'
PROTOCOL=Path('docs/protocols/phase_14r9_continuous_occlusion_empirical_retry.md')
CODE=('scripts/session_14r9_occlusion_study.py',
      'src/defensive_network_disruption/validation/r9_certificate_orchestration.py',
      'src/defensive_network_disruption/validation/r8_aggregation.py',
      'src/defensive_network_disruption/validation/r7_execution.py',
      'tests/test_session14r9_enforcement.py','tests/test_session14r9_study.py',
      'src/defensive_network_disruption/geometry/r7_representation.py')
CSV_FILES=('candidate_pair_comparison.csv','structural_correspondence.csv','receiver_corridor_ordering.csv','union_vs_max.csv','overlap_redundancy.csv','multi_edge_summary.csv','match_summary.csv')
PREACCESS_FILES=('preaccess_contract.json','retained_observation_certificate_gate.json',
    'runner_failure_oracles.json','runner_success_oracle.json','synthetic_acceptance.json',
    'evidence_type_summary.json','visual_qa.json')
FINAL_FILES=(*PREACCESS_FILES,'candidate_summary.json',*CSV_FILES,'synthetic_stress_summary.json',
             'qc.json','synthetic_field_comparison.svg')
KEYS=('candidate','comparison','summary','combination','metric')
STAT_KEYS=('mean','minimum','maximum','q05','q25','q50','q75','q95')
COLUMNS=(*KEYS,'alias','unit','aggregation_unit','represented_matches','states_total','states_assessable','states_unavailable','observations','source_observations','unavailable_reason',*STAT_KEYS)
...
try:
    ...
    from defensive_network_disruption.geometry.r7_representation import evaluate, evaluate_edge
    ...
    from defensive_network_disruption.validation.r7_execution import (
        Journal, Progress, Launch, durable_write, exposure_summary, read_journal,
        view, validate_persisted)
    from defensive_network_disruption.validation.r8_aggregation import (
        FamilySpec, PUBLIC_UNITS, registry_record, state_first, unit_for)
    from defensive_network_disruption.validation.r9_certificate_orchestration import retained_observation_gate
except Exception as exc:
    RUNTIME_ERROR=exc

TEST_COMMAND=('-m','unittest','discover','-s','tests','-p','test_session14r9_enforcement.py')
PROGRESS=None
...
def inherited_hashes():
    bindings={
        'docs/protocols/phase_13_defender_edge_influence.md':'03811fd9220722836394971af38395c4e683504fb6d591d91fcb8e0399e27b0b',
        'docs/protocols/phase_14ai_onset_aware_owner_certification.md':'d66158f7f866dcfdf3d6144f445e8f62196ec44b95610b44cbe5d9169cbfa7a8',
        'docs/protocols/phase_14aj_within_state_pair_aggregation.md':'32d7c1ad2e98563fc498fbb7f5db797d304c9117b7a2932e8b986d62a975cbd0',
        ...
        'src/defensive_network_disruption/validation/r7_execution.py':'2000018ad1c411b7e4f9935553be41cc848be89b6d072bc3c3560e5f500ebe54',
        'src/defensive_network_disruption/validation/pair_aggregation.py':'8e50d7d0821e28519ba38e0e911b1ad2eb6ffae9d3ec9b73f8b1dd2f14ae6ca0',
        'outputs/session14_pair_aggregation/manifest.json':'5adc0d0d07dfe459207ba245ae0a109b34ff0646917cffbf86d2706f0871b600',
        'uv.lock':'c9e4da0781770b9e72b8a5b10a7c514f2259b7e63dc1b67604ee481934db0a0c',
    }
    for path,expected in bindings.items():
        pv.require(digest(path)==expected,'historical_authority_changed')
    return bindings
...
def synthetic_acceptance(progress=lambda **kw:None):
    ...
    from defensive_network_disruption.validation.onset_owner_acceptance import topology_oracles
    topology=topology_oracles()
    pv.require(all(x['passed'] for x in topology),'onset_topology')
    engineering.extend(dict(fixture=x.get('fixture',x.get('name','onset_topology')),passed=x['passed']) for x in topology)
...
def render_synthetic(approve=False):
    ...
    if approve:
        ...
        write(OUT/'visual_qa.json',dict(schema_version=1,status='approved',
            svg_sha256=visual['svg_sha256'],byte_identical=True,width=1800,height=800,
            checks=['full_frame','no_clipping','no_overlapping_headings','legible_scale','consistent_extents']))
    ...
    try:
        ...
        def render():
            with matplotlib.rc_context({'svg.hashsalt':'phase14r9-fixed','font.family':'DejaVu Sans','font.size':11}):
                ...
                buf=io.StringIO();fig.savefig(buf,format='svg',metadata={'Date':None,'Creator':'Session 14R9 synthetic illustration'});plt.close(fig)
...
def check_binding(prod):
    ...
    pv.require(all(prod['preaccess_readiness'].values()) and prod['certificate_gate']['ready'],
               'r9_preaccess_readiness')

class Collect:
    def __init__(self):self.data=defaultdict(list);self.registry={}
    def add(self,file,alias,state,metric,values,*,candidate='none',comparison='none',summary='none',combination='none',unit='edge',source_observations=None):
        ...
        source=len(vals) if source_observations is None else source_observations
        vals=list(state_first(vals,unit))
        spec=FamilySpec(file,candidate,comparison,summary,combination,metric,unit)
        self.registry[key]=spec
        self.data[key].append((alias,state,vals,source))
    def tables(self):
        ...
        for key,records in sorted(self.data.items()):
            ...
            for group in (*ALIASES,'macro'):
                ...
                row=dict(candidate=candidate,comparison=comparison,summary=summary,combination=combination,
                         metric=metric,alias=group,unit=unit,
                         aggregation_unit='macro_match' if group=='macro' else 'match',represented_matches=len(bymatch),
                         ...
                         unavailable_reason=('no_states' if not selected else 'none' if len(valid)==len(selected) and all(len(v)==n for _,_,v,n in selected) else
                             ...
                             'no_eligible_pairs' if unit in ('state_pair_mean','state_pair_proportion') else 'zero_strength_or_ownerless'),**weighted_summary(values,weights))
...
def summarize_state(row,state_index,collector,across):
    ...
    for metric in ('nearest_intersects','nearest_equal','nearest_jaccard'):
        add('receiver_corridor_ordering.csv',metric,[x[metric] for x in edges])
    ...
    add('receiver_corridor_ordering.csv','raw_order_disagreement',[float((i,j) in discord) for i,j in validpairs],unit='state_pair_proportion')
    fields={};qc=[]
        ...
        for summary in ('receiver','segment'):
            ...
            if summary=='segment':
                for name,g in gaps.items():
                    ...
                    add('overlap_redundancy.csv','overlap_vs_'+name,[rho],candidate=candidate,summary=summary,unit='state')
            ...
            for combination,vals in (('union',uv),('maximum',mv)):
                ...
                for metric,vs in follows(vals,pairs).items():add('receiver_corridor_ordering.csv',metric,vs,unit='state_pair_proportion',**kw)
    ...
    for candidate in CANDIDATES:
        for combination in ('union','maximum'):
            ...
            add('receiver_corridor_ordering.csv','field_summary_spearman',[spearman(a,b)],unit='state',**kw)
            for metric,vs in order_categories(a,b).items():add('receiver_corridor_ordering.csv',metric,vs,unit='state_pair_proportion',**kw)
    for first,second in itertools.combinations(CANDIDATES,2):
        for summary in ('receiver','segment'):
            for combination in ('union','maximum'):
                ...
                add('candidate_pair_comparison.csv','paired_difference',[y-x for x,y in zip(a,b)],unit='state_pair_mean',**kw)
                add('candidate_pair_comparison.csv','candidate_pair_spearman',[spearman(a,b)],unit='state',**kw)
                for metric,vs in order_categories(a,b).items():add('candidate_pair_comparison.csv',metric,vs,unit='state_pair_proportion',**kw)
    return qc
    ...
    for candidate in CANDIDATES:
        ...
        for name in names:
            for alias in ALIASES:
                ...
                collector.add('overlap_redundancy.csv',alias,-1,'across_state_overlap_vs_'+name,[rho],
...
def expected_families():
    ...
    divergence='receiver_corridor_ordering.csv'; structure='structural_correspondence.csv'
    ...
    add(divergence,'raw_order_disagreement',unit='state_pair_proportion')
    for c in CANDIDATES:
        ...
        for gap in ('d2_d1','d3_d1'):add('overlap_redundancy.csv','overlap_vs_'+gap,c,summary='segment',unit='state')
        for descriptor in ('d2_d1','d3_d1',*[f'top{k}_{suffix}' for k in (1,2,3) for suffix in ('jaccard','distinct','maximum_involvement')]):
            add('overlap_redundancy.csv','across_state_overlap_vs_'+descriptor,c,summary='segment',unit='match')
        for combination in ('union','maximum'):
            for metric in ('field_summary_spearman',*orders):
                add(divergence,metric,c,'receiver_to_segment',combination=combination,
                    unit='state' if metric=='field_summary_spearman' else 'state_pair_proportion')
            ...
            for combination in ('union','maximum'):
                for metric in ('paired_difference','candidate_pair_spearman',*orders):
                    add('candidate_pair_comparison.csv',metric,comparison=a+'_to_'+b,summary=s,combination=combination,
                        unit='state_pair_mean' if metric=='paired_difference' else 'state' if metric=='candidate_pair_spearman' else 'state_pair_proportion')
    return result

# R6 stages are new; no historical runner is invoked for empirical execution.
def summary_contract_check():
    """Frozen within-state-first pair aggregation oracle; no data or fields."""
    collector=Collect()
    for index,values in ((0,[0.,1.]),(1,[1.,1.])):
        collector.add('candidate_pair_comparison.csv',ALIASES[0],index,'agreement',values,
                      comparison='isotropic_to_expanding',unit='pair')
    row=next(x for x in collector.tables()['candidate_pair_comparison.csv'] if x['alias']=='macro')
    observed={k:row[k] for k in STAT_KEYS}
    required=weighted_summary([.5,1.],[.5,.5])
    return dict(schema_version=1,fixture='two_state_pair_average',scope='synthetic_only',
                expected=required,observed=observed,passed=observed==required,
                defect='pair_percentiles_before_state_average')


def require_summary_contract():
    pv.require(summary_contract_check()['passed'],'pair_summary_aggregation_contract')


def validate_certificate_gate_record(record):
    required=('evidence_type','field_evaluation_performed','empirical_geometry_accessed',
        'certificate_scope','whole_integral_certified','unrelated_comparator_disagreement_resolved',
        'warning_preserved','certificate_lookup_attempted','exact_authority_match',
        'estimate_contained','width_accepted','piece_status','aggregate')
    pv.require(all(k in record for k in required),'certificate_gate_schema')
    pv.require(record['evidence_type']=='retained_observation_certificate_orchestration',
               'certificate_evidence_type')
    pv.require(not record['field_evaluation_performed'] and not record['empirical_geometry_accessed'],
               'certificate_gate_access')
    pv.require(record['certificate_scope']=='terminal_interval_8_only' and
               not record['whole_integral_certified'] and
               not record['unrelated_comparator_disagreement_resolved'],'certificate_scope')
    aggregate=record['aggregate']
    pv.require(record['warning_preserved'] and record['certificate_lookup_attempted'] and
               record['exact_authority_match'] and record['estimate_contained'] and
               record['width_accepted'],'certificate_observation')
    pv.require(aggregate['adaptive_warning_count']>=1 and
               aggregate['independently_certified_count']>=1 and
               aggregate['blocking_warning_count']==0,'certificate_aggregate')
    return True


def oracle_manifest(directory,status,stage,exception=None,gate=None):
    directory=Path(directory);directory.mkdir(parents=True,exist_ok=True)
    record=dict(schema_version=1,status=status,stage=stage,exception=exception,
        readiness=False if status=='failure' else bool(gate and gate.get('ready')),
        success=status=='success',empirical_states_opened=0,empirical_edges_opened=0,
        marker_exists=(directory/'attempt.marker').exists(),traceback=None)
    if exception is not None:record['traceback']=traceback.format_exc()
    durable_write(directory/'evidence.json',record)
    if gate is not None:durable_write(directory/'certificate_gate.json',gate)
    members={p.name:digest(p) for p in directory.iterdir() if p.is_file() and p.name!='manifest.json'}
    durable_write(directory/'manifest.json',dict(schema_version=1,status=status,outputs=members))
    return validate_oracle_package(directory)


def validate_oracle_package(directory):
    directory=Path(directory);manifest=json.loads((directory/'manifest.json').read_text())
    pv.require(set(manifest)=={'schema_version','status','outputs'},'oracle_manifest_schema')
    for name,expected in manifest['outputs'].items():
        pv.require(hashlib.sha256((directory/name).read_bytes()).hexdigest()==expected,'oracle_hash')
    evidence=json.loads((directory/'evidence.json').read_text())
    pv.require(evidence['marker_exists'] and evidence['empirical_states_opened']==0 and
               evidence['empirical_edges_opened']==0,'oracle_marker_access')
    if manifest['status']=='success':
        gate=json.loads((directory/'certificate_gate.json').read_text())
        pv.require(validate_certificate_gate_record(gate) and gate['ready'] and
                   evidence['readiness'] and evidence['success'],'oracle_success')
    else:
        pv.require(not evidence['readiness'] and not evidence['success'] and
                   evidence['exception'] and evidence['traceback'],'oracle_failure')
    return dict(status='valid_'+manifest['status'],validator_passed=True)


def oracle_entry(injection):
    marker=LOCAL/'attempt.marker';durable_write(marker,dict(stage='oracle',injection=injection))
    stage='package_import'
    try:
        if injection=='package_import':raise ImportError('attempted relative import with no known parent package')
        stage='post_marker'
        if injection=='post_marker':raise RuntimeError('injected_after_marker')
        stage='certificate_authority'
        if injection=='certificate_authority':
            from defensive_network_disruption.validation import independent_certificate_verifier as cv
            certificate,observation,_=cv.load_session14ar_certificate(ROOT)
            from dataclasses import replace
            cv.certify_warning(observation,replace(certificate,authority_sha256='0'*64))
        stage='certificate_gate'
        holder={}
        gate=retained_observation_gate(ROOT,publish=lambda record: holder.setdefault('validated',validate_certificate_gate_record(record)))
        stage='publication'
        if injection=='publication':raise RuntimeError('injected_publication_validation')
        pv.require(holder.get('validated') and gate['ready'],'gate_not_ready')
        provisional=LOCAL/'provisional'
        durable_write(provisional/'attempt.marker',dict(stage='oracle',injection=injection))
        initial=oracle_manifest(provisional,'success','closed',gate=gate)
        durable_write(LOCAL/'validator_receipt.json',initial)
        # Close the final package only after observing the provisional validator.
        final=oracle_manifest(LOCAL,'success','closed',gate=gate)
        print(encoded(final),end='')
    except BaseException as exc:
        oracle_manifest(LOCAL,'failure',stage,type(exc).__name__)
        if injection=='success':raise
        print(encoded(dict(status='valid_failure',stage=stage,exception=type(exc).__name__)),end='')


def runner_oracles():
    results=[]
    for injection in ('package_import','post_marker','certificate_authority','publication','success'):
        namespace=LOCAL/'runner_oracles'/injection
        env={**os.environ,'SESSION14R9_OUTPUT':str(namespace)}
        result=subprocess.run([sys.executable,str(Path(__file__).resolve()),'oracle','--inject',injection],
                              cwd=ROOT,env=env,text=True,capture_output=True)
        pv.require(result.returncode==0,'oracle_subprocess_'+injection)
        validated=validate_oracle_package(safe(namespace)/'local')
        expected='valid_success' if injection=='success' else 'valid_failure'
        pv.require(validated['status']==expected,'oracle_status_'+injection)
        rerun=subprocess.run([sys.executable,str(Path(__file__).resolve()),'oracle','--inject',injection],
                             cwd=ROOT,env=env,text=True,capture_output=True)
        pv.require(rerun.returncode!=0,'oracle_rerun_not_rejected_'+injection)
        results.append(dict(injection=injection,status=validated['status'],runner_caught=True,
                            validator_passed=validated['validator_passed'],rerun_rejected=True))
    return results
...
def preflight():
    ...
    print('R9 protocol and historical authority verified; no development input opened')
...
def verify_production():
    ...
    try:
        durable_write(safe(LOCAL/'retained-observation.marker'),dict(stage='retained_observation_gate'))
        ...
        oracles=runner_oracles()
        observed={}
        certificate_gate=retained_observation_gate(
            ROOT,publish=lambda record: observed.setdefault('validated',validate_certificate_gate_record(record)))
        pv.require(observed.get('validated') and certificate_gate['ready'],'retained_certificate_gate')
        durable_write(safe(LOCAL/'synthetic-acceptance.marker'),dict(stage='synthetic_acceptance'))
        ...
        preaccess_inputs=dict(retained_observation_certificate_orchestration=certificate_gate['ready'],
            actual_runner_failure_publication=all(x['status']=='valid_failure' for x in oracles[:-1]),
            import_failure_publication=oracles[0]['status']=='valid_failure',
            validator_derived_success=oracles[-1]['status']=='valid_success',
            synthetic_108_366_399=gate.ready,integrity=True,zero_empirical_access=True)
        pv.require(all(preaccess_inputs.values()),'preaccess_readiness_false')
        write(AUTH/'production.json',dict(schema_version=1,status='passed',protocol=digest(PROTOCOL),
              ...
              readiness=asdict(gate),preaccess_readiness=preaccess_inputs,
              certificate_gate=certificate_gate,runner_oracles=oracles,
              cases=rows,engineering=engineering,tests=tests))
        write(OUT/'preaccess_contract.json',dict(schema_version=1,status='passed',
            requirements=preaccess_inputs,ready=all(preaccess_inputs.values()),
            certificate_scope='terminal_interval_8_only',whole_integral_gates_unchanged=True))
        write(OUT/'retained_observation_certificate_gate.json',certificate_gate)
        write(OUT/'runner_failure_oracles.json',dict(schema_version=1,
            records=oracles[:-1],all_valid=all(x['status']=='valid_failure' for x in oracles[:-1])))
        write(OUT/'runner_success_oracle.json',dict(schema_version=1,**oracles[-1],
            success_derived_from_validator=oracles[-1]['validator_passed']))
        write(OUT/'synthetic_acceptance.json',dict(schema_version=1,
            evidence_type='synthetic_numerical_evidence',field_evaluation_performed=True,
            cases=len(rows),references=sum(len(x['errors']) for x in rows),
            permutations=sum(x['permutations'] for x in rows),readiness=asdict(gate),passed=gate.ready))
        write(OUT/'evidence_type_summary.json',dict(schema_version=1,
            certificate_orchestration_evidence='retained_observation',
            synthetic_numerical_evidence='evaluated_synthetic_fields',
            empirical_geometry_access_before_unlock=0,evidence_types_merged=False))
        ...
        print('R9 retained certificate gate and 108/366/399 synthetic acceptance passed')
    except BaseException as exc:
...
def empirical_authority():
    ...
    for path in (AUTH/'production.json',AUTH/'visual_approval.json',OUT/'synthetic_field_comparison.svg',
                 *(OUT/name for name in PREACCESS_FILES)):
        committed(path)
...
def analyze():
    ...
    with safe(LOCAL/'prepared.jsonl').open() as source:
        for index,line in enumerate(source):
            ...
            PROGRESS.retain_and_complete_edges(str(index));PROGRESS.complete_state(str(index));done+=1
    ...
    registry=[registry_record(collector.registry[key]) for key in sorted(collector.registry)]
    write(staging/'pair_aggregation_summary.json',dict(schema_version=1,
        status='state_first_pair_aggregation_applied',families=registry,
        pair_mean_families=sum(x['unit']=='state_pair_mean' for x in registry),
        pair_proportion_families=sum(x['unit']=='state_pair_proportion' for x in registry),
        raw_source_counts_preserved=True,published_observations_are_state_summaries=True))
...
def close_failure(stage,error,*unused):
    ...
    original=safe(LOCAL/'original_failure.json')
    if not original.exists():
        durable_write(original,dict(stage=stage,exception=type(error).__name__,traceback=trace,
            context=None if PROGRESS is None else PROGRESS.context,
            journal_head=None if PROGRESS is None else PROGRESS.journal.previous))
    try:
        ...
        if not emergency.exists():durable_write(emergency,dict(stage=stage,exception=type(error).__name__,
            ...
            publication_traceback=traceback.format_exc(),snapshot=None))
        raise
...
def publish_package(authority,status,stage,exception,staging=None):
    ...
    if status=='success':
        ...
        pair=load(staging/'pair_aggregation_summary.json')
        pv.require(pair['status']=='state_first_pair_aggregation_applied' and pair['families'],'pair_aggregation_summary')
    ...
    if staging:
        for name in CSV_FILES+('candidate_summary.json','pair_aggregation_summary.json'):
            atomic(package/name,safe(staging/name).read_text())
    if PROGRESS is not None:
        from defensive_network_disruption.validation import numerical_failure_publication as pub
        observed=pub.authority(PROGRESS.journal.path,PROGRESS.journal.previous)
        acceptance=pub.package(observed,accepted=status=='success',checks={'complete_processing':status=='success'})
        for member in ('qc','manifest','evidence'):
            durable_write(safe(package/'numerical_publication'/(member+'.json')),acceptance)
    durable_write(safe(package/'lifecycle_summary.json'),authority)
    durable_write(safe(package/'exposure_summary.json'),
        exposure_summary(safe(LOCAL/'journal.jsonl'),expected_head=None if PROGRESS is None else PROGRESS.journal.previous))
    ...
    for n in files:
        if n in FINAL_FILES:atomic(OUT/n,safe(package/n).read_text())
    ...
    for name in CSV_FILES:
        ...
        if name in CSV_FILES:
            ...
            for row in rows:
                ...
                pv.require(row['unit'] in PUBLIC_UNITS,'unit')
                pv.require(row['aggregation_unit'] in ('match','macro_match'),'aggregation_unit')
                pv.require(row['aggregation_unit']==('macro_match' if row['alias']=='macro' else 'match'),'aggregation_alias')
...
def publication_check():
    ...
    if m['status']=='success':
        pair=load(package/'pair_aggregation_summary.json')
        pv.require(pair['status']=='state_first_pair_aggregation_applied','pair_summary_status')
    print('R9 persisted publication validated: '+m['status'])
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('command',choices=('preflight','verify-production','render-synthetic','run','publication-check','oracle'))
    parser.add_argument('--approve-visual',action='store_true')
    parser.add_argument('--inject',choices=('package_import','post_marker','certificate_authority','publication','success'))
    ...
    try:
        with warnings.catch_warnings():
            ...
            {'preflight':preflight,'verify-production':verify_production,
             ...
             'oracle':lambda:oracle_entry(args.inject or 'success')}[args.command]()
    except BaseException as exc:
...
def startup_failure(error):
    ...
    authority=dict(schema_version=2,mode='pre_access',snapshot=None,
    ...
    atomic(OUT/'qc.json',safe(package/'qc.json').read_text())
    write(OUT/'manifest.json',dict(**base,start=START,protocol=digest(PROTOCOL),implementation=code_hashes(),
        environment={'unavailable':'runtime_import_failure'},outputs={'qc.json':digest(OUT/'qc.json')},
        unavailable=[n for n in FINAL_FILES if n!='qc.json'],
        private_evidence_sha256=digest(package/'evidence.json'),private_manifest_sha256=digest(package/'manifest.json')))
