#!/usr/bin/env python3
"""One-shot Session 14R9L label-free empirical execution."""
...
START = "3b8a6e578a9ca0c5be4b5836c00bafce1ef2c043"
...
PROTOCOL = Path("docs/protocols/phase_14r9l_empirical_representation_retry.md")
OUT = Path(os.environ.get("SESSION14R9L_OUTPUT", "outputs/continuous_occlusion_empirical_retry_r9l"))
LOCAL = OUT / "local"
R9 = Path("outputs/continuous_occlusion_empirical_retry_r9")
R9A = Path("outputs/continuous_occlusion_empirical_retry_r9a")
TEST_COMMAND = ("-m", "unittest", "discover", "-s", "tests", "-p", "test_session14r9l.py")
INHERITED_PUBLIC = (
...
CODE = (
    "scripts/session_14r9l_empirical_representation_retry.py",
...
def close_failure(stage: str, error: BaseException) -> None:
    ...
    try:
        ...
        authority = retain_authority(linear_review(PROGRESS.journal.path, expected_head=PROGRESS.journal.previous))
...
def run() -> None:
    ...
    try:
        ...
        print("R9L empirical execution closed and publication validated")
