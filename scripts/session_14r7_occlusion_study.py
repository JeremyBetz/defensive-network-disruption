#!/usr/bin/env python3
"""Governed Session 14R7, explicit label-free geometry and immutable stages."""
...
START='3bd02c10427bd35d1c45ea1e203c68ccd141bf75'
...
OUT=Path('outputs/continuous_occlusion_retry_14r7'); LOCAL=OUT/'local'; AUTH=OUT/'authority'
PROTOCOL=Path('docs/protocols/phase_14r7_continuous_occlusion_empirical_retry.md')
CODE=('scripts/session_14r7_occlusion_study.py',
      'src/defensive_network_disruption/validation/r7_execution.py',
      'tests/test_session14r7_enforcement.py','tests/test_session14r7_study.py',
...
TEST_COMMAND=('-m','unittest','discover','-s','tests','-p','test_session14r7_enforcement.py')
PROGRESS=None
...
def inherited_hashes():
    text=safe(PROTOCOL).read_text()
    bindings=dict(re.findall(r"\| `([^`]+)` \| `([0-9a-f]{64})` \|",text))
    ...
    try:
        ...
        def render():
            with matplotlib.rc_context({'svg.hashsalt':'phase14r-fixed','font.family':'DejaVu Sans','font.size':11}):
                ...
                buf=io.StringIO();fig.savefig(buf,format='svg',metadata={'Date':None,'Creator':'Session 14R7 synthetic illustration'});plt.close(fig)
