#!/usr/bin/env python3
"""One-shot Session 14R9N label-free empirical execution."""
...
START = "f308aa6072486a56738378722d59fa96d0de47a7"
...
PROTOCOL = Path("docs/protocols/phase_14r9n_empirical_representation_retry.md")
OUT = Path(os.environ.get("SESSION14R9N_OUTPUT", "outputs/continuous_occlusion_empirical_retry_r9n"))
LOCAL = OUT / "local"
R9 = Path("outputs/continuous_occlusion_empirical_retry_r9")
R9A = Path("outputs/continuous_occlusion_empirical_retry_r9a")
TEST_COMMAND = ("-m", "unittest", "discover", "-s", "tests", "-p", "test_session14r9n.py")
...
CODE = (
    "scripts/session_14r9n_empirical_representation_retry.py",
...
def verify_inherited() -> dict:
    ...
    r9l = json.loads(safe(Path("outputs/continuous_occlusion_empirical_retry_r9l/manifest.json")).read_text())
    if (r9l.get("status") != "failure"
            or r9l.get("progress_authority", {}).get("exposure", {}).get("states_opened") != 0
            or r9l.get("progress_authority", {}).get("exposure", {}).get("edges_opened") != 0):
        raise RuntimeError("r9l_historical_status")
...
def ci_expectation() -> CIExpectation:
    return CIExpectation(
        ...
        runner_implementation_sha256=sha(Path("scripts/session_14r9n_empirical_representation_retry.py")),
...
def close_failure(stage: str, error: BaseException, controller: FailureController) -> None:
    if safe(OUT / "manifest.json").exists():
        return
    record = controller.capture(error, stage, context=None if PROGRESS is None else PROGRESS.context)
    if PROGRESS is None:
        return
    try:
        PROGRESS.failure(record["journal_stage"], error)
        ...
        controller.publication_failure(publication_error)
...
def run() -> None:
    ...
    try:
        ...
        print("R9N empirical execution closed and publication validated")
    except BaseException as error:
        close_failure(stage, error, controller)
