#!/usr/bin/env python3
"""Governed Session 14R5, explicit label-free geometry and immutable stages."""
...
START='557644ccff2255b78311acf3e2e17334d61ddf06'
...
OUT=Path('outputs/continuous_occlusion_retry_14r5'); LOCAL=OUT/'local'; AUTH=OUT/'authority'
PROTOCOL=Path('docs/protocols/phase_14r5_continuous_occlusion_empirical_retry.md')
CODE=('scripts/session_14r5_occlusion_study.py',
      'src/defensive_network_disruption/validation/r5_persistence.py',
      'tests/test_session14r5_enforcement.py','tests/test_session14r5_study.py')
...
FINAL_FILES=('candidate_summary.json',*CSV_FILES,'synthetic_stress_summary.json','lifecycle_summary.json','qc.json','synthetic_field_comparison.svg')
...
try:
    ...
    from defensive_network_disruption.validation.r5_persistence import (
        Journal, Progress, Launch, durable_write, read_journal, receipts, view, validate_persisted)
except Exception as exc:
    RUNTIME_ERROR=exc

TEST_COMMAND=('-m','unittest','discover','-s','tests','-p','test_session14r5_enforcement.py')
PROGRESS=None
    ...
    try:
        ...
        def render():
            with matplotlib.rc_context({'svg.hashsalt':'phase14r-fixed','font.family':'DejaVu Sans','font.size':11}):
                ...
                buf=io.StringIO();fig.savefig(buf,format='svg',metadata={'Date':None,'Creator':'Session 14R5 synthetic illustration'});plt.close(fig)
...
# R5 stages are new; no historical runner is invoked for empirical execution.
def preflight():
    history();committed(PROTOCOL)
    pv.require(Path(sys.prefix).resolve()==safe(Path('.venv')).resolve(),'project_environment_required')
    print('R5 protocol and historical authority verified; no development input opened')
...
def verify_production():
    ...
    try:
        ...
        print('108/366/399 assembled R5 acceptance passed')
...
def access_receipt(state,edge,callback):
    j=PROGRESS.journal;attempt=str(j.count)
    j.append('access_attempt',attempt=attempt,state=state,edge=edge)
    result=callback()
    j.append('access_opened',attempt=attempt)
    return result
...
def prepare():
    ...
    with safe(POP).open() as source,safe(LOCAL/'prepared.jsonl').open('x') as dest:
        for index,line in enumerate(source):
            state=str(index)
            # Opening the lexical geometry record is exposure even if projection later fails.
            raw=access_receipt(state,None,lambda:line)
            key,row=project_line(raw)
            pv.require(key not in seen,'duplicate_observation');seen.add(key)
            edges=tuple(str(i) for i in range(len(row['receivers'])))
            PROGRESS.transition('discover_state',state,edges)
            PROGRESS.transition('open_state',state)
            for i in edges:
                access_receipt(state,i,lambda i=i:row['receivers'][int(i)])
                PROGRESS.transition('open_edge',state,i)
            count[row['alias']]+=1;n+=1
            dest.write(json.dumps(row,sort_keys=True,allow_nan=False)+'\n');dest.flush();os.fsync(dest.fileno())
            PROGRESS.transition('prepare_state',state)
...
def analyze():
    ...
    with safe(LOCAL/'prepared.jsonl').open() as source:
        for index,line in enumerate(source):
            row=json.loads(line);PROGRESS.transition('start_state_evaluation',str(index))
            ...
            PROGRESS.transition('complete_state',str(index));done+=1
...
def publication_check():
    ...
    print('R5 persisted publication validated: '+m['status'])
...
def run():
    ...
    def access(context):
        ...
        PROGRESS.transition('authorize_access');prepare();staging,qc=analyze()
        # Processing success remains separate from successful publication.
        PROGRESS.transition('succeed')
...
def startup_failure(error):
    ...
    null_hash=hashlib.sha256(b'null\n').hexdigest()
    authority=dict(schema_version=1,mode='pre_access',lifecycle_initialized=False,snapshot=None,
        snapshot_sha256=null_hash,journal_sha256=None,access=dict(states_opened=0,edges_opened=0,attempts=0,unresolved_attempts=0,states_with_unresolved_edge_exposure=0),
        candidate_calls=dict(started=0,completed=0),transition_unresolved=False)
