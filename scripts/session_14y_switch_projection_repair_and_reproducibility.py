...
START = "890c9142f59e5c3c760d70090e8d18ccc7e0b974"
TAG = "f00690c05d8c1c6db308a65bd311c43f9a8ef2fa"
PROTOCOL = Path("docs/protocols/phase_14y_switch_projection_repair_and_reproducibility.md")
OUT = Path("outputs/cross_platform_vector_reproducibility_14y")
LOCAL = OUT / "local"
...
CODE = (Path("scripts/session_14y_switch_projection_repair_and_reproducibility.py"),
        Path("src/defensive_network_disruption/geometry/vector_reproducibility.py"),
        Path("src/defensive_network_disruption/geometry/switch_diagnostic_projection.py"),
        Path("tests/test_session14y_switch_projection.py"),
        Path(".github/workflows/session14y-diagnostic.yml"))
...
def route(candidate, origin, receiver, defenders):
    ...
    return {"partitions":list(parts),"switches":[project_verified_switch(x) for x in envelope.switches],
...
def preflight():
    ...
    print("Session 14y preflight passed; synthetic diagnostic only")
...
def publication_check():
    ...
    print("Session 14y publication checks passed")
